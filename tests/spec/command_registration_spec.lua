...
describe('command registration', function()
  ...
  describe('command execution', function()
    ...
    it('should accept arguments when ClaudeCode command is executed', function()
      local toggle_args = nil

      -- Create a mock claude_code with toggle that captures args
      local claude_code_with_args = {
      ...
      -- Register commands with the new mock
      commands_module.register_commands(claude_code_with_args)

      -- Find the ClaudeCode command and execute with args
      for _, cmd in ipairs(registered_commands) do
        if cmd.name == 'ClaudeCode' then
          -- Simulate command execution with args
          cmd.callback({ args = '/usage' })
          break
        end
      end

      assert.equals('/usage', toggle_args, 'Arguments should be passed to toggle function')
    end)
  end)
end)
